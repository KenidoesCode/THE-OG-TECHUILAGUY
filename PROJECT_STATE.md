...
current commit (33 suites, 797 assertions, 0 failures). See
...
| **Packet inspector + timeline session** (NetLab product build Phase 1 — the UI-independent core a future timeline/inspector UI would call into) | TESTED | `24-network-simulator/netlab/packet_inspector.*`, `simulation_session.*`; `tests/packet_inspector_test.sh` + `tests/simulation_session_test.sh` — decodes real captured ARP/IPv4/ICMP frames (built by the actual simulation, not hand-crafted fixtures) into human-readable summaries/fields using only `06-networking`'s existing parse functions; a decoded frame's fields are directly verified to match the real values the live simulation used (no fabricated visual state); malformed/unsupported frames fail decoding cleanly; `SimulationSession` steps forward/backward through a real multi-event cross-router timeline in exact recorded/reversed order, supports `jumpTo`/`reset`, and stepping two independently-run but identical simulations is proven to yield identical events at every step (deterministic replay). See [`docs/ADR/0029-netlab-packet-inspector-and-timeline-session.md`](docs/ADR/0029-netlab-packet-inspector-and-timeline-session.md) for explicit non-goals (no browser UI/rendering/WASM build, no live/streaming simulation, no explanation text beyond a factual one-line summary, no timeline branching/comparison) |
| **Browser-foundation facade** (`NetLabFacade` — pre-WASM API consolidation, Phase A prerequisite) | TESTED | `24-network-simulator/netlab/facade.*`; `tests/facade_test.sh` — 14 hosted assertions incl. `buildReferenceScenario()` reproducing the project's own verified PC1→Switch→Router→Switch→PC2 scenario through one entry point, the facade's stepped session proven to reproduce the EXACT event order/bytes a directly-built `SimulationSession` over the same timeline would (the facade doesn't silently alter anything it composes), `currentEventDecoded()` reflecting the real destination IP the facade's own `runPing()` call used, and the mission evaluator working identically through the facade. See [`docs/ADR/0030-netlab-browser-foundation-facade.md`](docs/ADR/0030-netlab-browser-foundation-facade.md) |
| **WebAssembly build / browser UI** — explicitly BLOCKED, not merely unstarted | PLANNED (blocked) | this execution environment has no Emscripten toolchain (`emcc` not found); `NetLabFacade` above is deliberately shaped to be bindable once a toolchain is available, but no WASM compilation, Embind/C-ABI binding code, or browser application shell (HTML/CSS/JS, topology renderer, packet visualization) has been written or could be verified in this environment — this is an honestly-disclosed toolchain gap, not a silent omission |
| DHCP/DNS/TCP/UDP behavior, multi-hop routing, VLANs/NAT/firewalls, latency/loss/bandwidth simulation, visual topology editor, additional missions/grading/XP/skill tree/classroom mode, multiplayer, save/load, AI tutor | PLANNED | none of these exist yet |
